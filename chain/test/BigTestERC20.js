...
describe("SparklaunchSale", function() {
  ...
  const TOKEN_PRICE_IN_ERC20 = (10 ** DECIMALS).toString();
  ...
  const PARTICIPATION_VALUE = 200 * REV;
  ...
  async function participate(params) {
    ...
    await TestToken.connect(registrant).approve(SparklaunchSale.address, value);
    return await SparklaunchSale.connect(registrant).participate(value, participationRound);
  }
  ...
  async function depositTokens() {
    ...
    await SparklaunchSale.depositSaleTokens();
  }
  ...
  beforeEach(async function() {
    ...
    const saleContract = await ethers.getContractFactory("SparklaunchSaleERC20");
    await SalesFactory.setFeeAddr(cedric.address);
    await SalesFactory.setServiceFee(100);
    await SalesFactory.deployNormalSaleERC20();
    const SparklaunchSaleFactory = await ethers.getContractFactory("SparklaunchSaleERC20");
  ...
  context("Setup", async function() {
    ...
    describe.only("Set sale parameters", async function() {
      it("Should set the sale parameters", async function() {
        ...
        const tokenPriceInERC20 = TOKEN_PRICE_IN_ERC20;
        ...
        expect(sale.saleToken).to.equal(token);
        expect(sale.isCreated).to.be.true;
        expect(sale.saleOwner).to.equal(saleOwner);
        expect(sale.saleTokenPriceInERC20).to.equal(tokenPriceInERC20);
        expect(sale.softCap).to.equal(softCap);
        expect(sale.hardCap).to.equal(hardCap);
        expect(await SparklaunchSale.tokenERC20()).to.equal(erc20);
      ...
      it("Should emit SaleCreated event when parameters are set", async function() {
        ...
        .withArgs(saleOwner, tokenPriceInERC20, saleEnd, hardCap, softCap, erc20);
      });
      ...
      it("Should not set sale parameters if token price is 0", async function() {
        // Then
        await expect(setSaleParams({tokenPriceInERC20: 0})).to.be.revertedWith("Invalid input.");
      });
    ...
    describe.only("Edge Cases & Miscellaneous", async function () {
    ...
    describe.only("Deposit tokens", async function() {
      ...
      it("Should not allow non-sale owner to deposit tokens", async function() {
        ...
        // Then
        await expect(SparklaunchSale.depositSaleTokens()).to.be.revertedWith("Restricted to sale owner.");
      });
  ...
  context("Participation", async function() {
    describe.only("Participate", async function() {
      it("Should allow user to participate", async function() {
        ...
        await TestToken.approve(SparklaunchSale.address, PARTICIPATION_VALUE);
        // When
        await participate();
        ...
        expect(sale.totalSaleTokensSold).to.equal(Math.floor(PARTICIPATION_VALUE / TOKEN_PRICE_IN_ERC20 * MULTIPLIER));
        expect(sale.totalERC20Raised).to.equal(PARTICIPATION_VALUE);
        expect(isParticipated).to.be.true;
        expect(participation.amountBought).to.equal(Math.floor(PARTICIPATION_VALUE / TOKEN_PRICE_IN_ERC20 * MULTIPLIER));
      ...
      it("Should allow multiple users to participate", async function() {
        ...
        await TestToken.approve(SparklaunchSale.address, PARTICIPATION_VALUE);
        await TestToken.transfer(alice.address, PARTICIPATION_VALUE);
        await TestToken.connect(alice).approve(SparklaunchSale.address, PARTICIPATION_VALUE);
        // When
        await participate();
        ...
        expect(sale.totalSaleTokensSold).to.equal(Math.floor(2 * PARTICIPATION_VALUE / TOKEN_PRICE_IN_ERC20 * MULTIPLIER));
        expect(sale.totalERC20Raised).to.equal(BigNumber.from(PARTICIPATION_VALUE).mul(2));
        expect(isParticipatedDeployer).to.be.true;
        expect(isParticipatedAlice).to.be.true;
        expect(participationDeployer.amountBought).to.equal(Math.floor(PARTICIPATION_VALUE / TOKEN_PRICE_IN_ERC20 * MULTIPLIER));
        expect(participationDeployer.tierId).to.equal(PARTICIPATION_ROUND);
        // expect(participationDeployer.isWithdrawn).to.be.false;
        expect(participationAlice.amountBought).to.equal(Math.floor(PARTICIPATION_VALUE / TOKEN_PRICE_IN_ERC20 * MULTIPLIER));
      ...
      it("Should not participate in a round that has not started", async function() {
        ...
        // Then
        await expect(participate({participationRound: 1}))
          .to.be.revertedWith("Your round haven't started yet");
      });
      ...
      it("Should emit TokensSold event", async function() {
        ...
        // Then
        await expect(participate()).to.emit(SparklaunchSale, "TokensSold").withArgs(deployer.address, Math.floor(PARTICIPATION_VALUE / TOKEN_PRICE_IN_ERC20 * MULTIPLIER));
      });
    ...
    describe.only("Withdraw tokens", async function() {
      it("Should withdraw user's tokens", async function() {
        // Given
        await runFullSetup();
        const withdrawAmount = (PARTICIPATION_VALUE / TOKEN_PRICE_IN_ERC20)  * MULTIPLIER;
      ...
      it("Should emit TokensWithdrawn event", async function() {
        ...
        // Then
        await expect(SparklaunchSale.withdraw()).to.emit(SparklaunchSale, "TokensWithdrawn").withArgs(deployer.address, Math.floor(PARTICIPATION_VALUE / TOKEN_PRICE_IN_ERC20  * MULTIPLIER));
      });

    });

    describe.only("Withdraw earnings and leftover", async function() {
      it("Should withdraw sale owner's earnings and leftovers", async function() {
        ...
        const previousBalance = await TestToken.balanceOf(deployer.address);
        ...
        const contractBalance = await TestToken.balanceOf(SparklaunchSale.address);
        ...
        expect(currentTokenBalance).to.equal(previousTokenBalance.add((AMOUNT_OF_TOKENS_TO_SELL - PARTICIPATION_VALUE / TOKEN_PRICE_IN_ERC20)));
        expect(contractBalance).to.equal(0);
        expect(contractTokenBalance).to.equal(PARTICIPATION_VALUE / TOKEN_PRICE_IN_ERC20);
      });
      ...
      it("Should burn leftover if requested", async function() {
        ...
        expect(contractBalance).to.equal(0);
        expect(contractTokenBalance).to.equal(PARTICIPATION_VALUE / TOKEN_PRICE_IN_ERC20);
        expect(burnedTokenBalance).to.equal(AMOUNT_OF_TOKENS_TO_SELL - PARTICIPATION_VALUE / TOKEN_PRICE_IN_ERC20);
      });

      //TODO:
      it("Should not crash if leftover is 0", async function() {
        // Given
        await runFullSetup({hardCap: Math.floor(PARTICIPATION_VALUE / TOKEN_PRICE_IN_ERC20 * MULTIPLIER)});
        ...
        expect(currentTokenBalance).to.equal(previousTokenBalance);
        expect(contractBalance).to.equal(0);
        expect(contractTokenBalance).to.equal(PARTICIPATION_VALUE / TOKEN_PRICE_IN_ERC20 * MULTIPLIER);
      });
    ...
    describe.only("Get current round", async function() {
  ...
  context("Finish Sale", async function(){
    ...
    describe.only("Withdraw after finish sale if sale successful", async function(){
      ...
      it("Make sure if sale successful sale owner can withdraw earnings+leftover", async function(){
        ...
        const previousBalance = await TestToken.balanceOf(deployer.address);
        const previousBalanceCedric = await TestToken.balanceOf(cedric.address);
        ...
        // Then
        const currentBalance = await TestToken.balanceOf(deployer.address);
        const currentBalanceCedric = await TestToken.balanceOf(cedric.address);
      ...
      it("Make sure if sale successful users can not withdraw deposited ERC20", async function(){
        ...
        await participate({participationAmount: 100 * REV})
    ...
    describe("Withdraw after finish sale if sale cancelled", async function(){
      ...
      it("Make sure if sale cancelled users can withdraw deposited ERC20", async function(){
        // Given
        await runFullSetup();
